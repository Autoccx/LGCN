...
def build_optimizer(args, model):
    ve_module = getattr(model, 'visual_extractor', None)
    ve_params_all = list(ve_module.parameters()) if ve_module is not None else []
    ve_ids = set(map(id, ve_params_all))
    ve_params = [p for p in ve_params_all if p.requires_grad]
    ed_params = [p for p in model.parameters() if id(p) not in ve_ids and p.requires_grad]

    param_groups = []
    if len(ve_params) > 0:
        param_groups.append({'params': ve_params, 'lr': args.lr_ve})
    if len(ed_params) > 0:
        param_groups.append({'params': ed_params, 'lr': args.lr_ed})
