...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        # Clean Transformer decoder: CMN textual memory querying is intentionally disabled.
...
class BaseCMN(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        # Clean decoder setting: keep the original Transformer encoder-decoder,
        # but do not create or use CMN memory_matrix / memory querying.
        self.cmn = None
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        # VLPGF feeds concatenated memory tokens such as V* + I* (+ P*) into BaseCMN.
        # Their masks may contain valid tokens after padded positions, for example:
        # [visual all-1] + [indication 1/0] + [prior 1/0].
        # Therefore, do not use the original clip_att()/pack_wrapper() path, which
        # assumes that all valid tokens are before all padding tokens.
        if att_masks is not None:
            assert att_feats.size(1) == att_masks.size(1), (
                f"att_feats length {att_feats.size(1)} != att_masks length {att_masks.size(1)}"
            )
        ...
        # Clean encoder setting: CMN visual memory querying is intentionally disabled.
