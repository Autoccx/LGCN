...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        min_length = int(opt.get('min_length', 0) or 0)
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    # prevent early stop: do not allow EOS before min_length tokens
                    if min_length > 0 and (t - divm) < min_length:
                        logprobs = logprobs.clone()
                        logprobs[:, self.eos_idx] = float('-inf')
                        unaug_logprobs = unaug_logprobs.clone()
                        unaug_logprobs[:, self.eos_idx] = float('-inf')
